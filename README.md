...
*(Your exact scores will differ slightly — recency is a live feature, so the same fixture ages as days pass. That's deliberate: a lead list that doesn't decay is lying to you.)*

## Why the baseline is the point

The scorer is a transparent rule engine, and that's a deliberate choice rather than a placeholder for something smarter.
...
**Before I'd trust an LLM scorer in production**, I'd want, against a held-out labeled set: rank correlation with the baseline (does it broadly agree, and where doesn't it?), tier-agreement rate (how often does a lead move tiers, and is that movement defensible?), stability under re-runs at temperature 0, cost and latency per thousand records, and a manual read of every case where the two scorers disagree by more than one tier — because those disagreements are where you find out whether the model learned something real or just learned to be confidently vague.
...
| Equity | 20 | Scaled by estimated equity ratio, clamped to 0–1 |
...
Feature extraction as pure functions · deterministic baseline before ML · interface seams for swappable components · explainable scoring · evaluation criteria for model replacement · boundary-condition testing · HTML escaping in generated reports · stable sort ordering for diffable output.
